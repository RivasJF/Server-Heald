...
import { RemoveClinicUseCase } from '../../../../../src/clinic/use-case/remove-clinic.use-case';

describe('RemoveClinicUseCase', () => {
  let useCase: RemoveClinicUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new RemoveClinicUseCase(clinicRepository);
  });

  it('elimina clínica existente', async () => {
    ...
    clinicRepository.delete.mockResolvedValue(clinic);
  ...
  it('lanza NotFoundException cuando no existe clínica', async () => {
    ...
    await expect(useCase.execute('clinic-404')).rejects.toThrow(NotFoundException);
  });
});
