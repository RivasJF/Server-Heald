...
@Injectable()
export class FindNearbyClinicPaginationUseCase {
  ...
  async execute(lat: number, lng: number, radius = 5000, page: number, pageSize: number) {
    const { minLatitude, maxLatitude, minLongitude, maxLongitude } =
      this.getBoundingBox(lat, lng, radius);

    const candidateClinics = await this.clinicRepository.findByCoordinatesRangePagination(
      minLatitude,
      maxLatitude,
      minLongitude,
      maxLongitude,
